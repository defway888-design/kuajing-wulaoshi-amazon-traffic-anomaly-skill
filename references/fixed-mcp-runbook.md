...
本文中的“领星 MCP”“卖家精灵 MCP”“SIF MCP”是数据源身份和业务边界，不是必须固定一致的本机 MCP server id 或工具前缀。正式运行前必须在当前使用者自己的 AI/智能体运行环境与 MCP 环境中解析实际可用工具，并将解析结果写入内部运行状态。
...
- 如当前运行环境可发现本地全局 Skill、项目规则或知识库 `kuajing-wulaoshi-sellersprite-mcp-database`，先通过当前 AI/智能体的平台资源列表读取其数据库说明；不得假定该规则资源位于任何固定目录。该数据库说明不能覆盖本文件为流量异动写死的工具、参数、字段和阻塞条件。
...
方向性排除也必须写入内部状态。若某个因素在业务上只支持相反流量方向，可以不调用其高频 MCP 接口，但必须把该验证项标记为 `verified`，并在 `evidence` 中写明 `direction_excluded=true` 与排除原因。方向性排除不是未验证，也不能输出为可能因素。

## 2.1 高频接口限流与缓存

所有 MCP 调用都必须先做调用计划，禁止因为父商品子体数量大而无控制地循环请求。

通用限流：

- 同一 MCP 的同一功能域禁止并发调用，必须串行执行。
- 每次调用前先检查本次运行缓存；缓存键固定为：`MCP 名称 + 工具名 + 市场 + ASIN/关键词 + 时间窗口 + returnFields/字段集 + 其他影响结果的参数`。
- 相同缓存键已经成功返回时，不得重复调用接口。
- 遇到“每分钟访问已达上限”“rate limit”“too many requests”或同义错误时，立即停止该功能域后续调用，等待至少 70 秒后从失败请求继续重试；同一请求最多重试 2 次。
- 重试仍触发限流时，本验证项标记为 blocked，`blocked_reason` 写明触发限流的 MCP、工具/功能域、已完成数量、剩余数量和建议稍后重跑；不得重启全流程或继续撞限。

卖家精灵限流：

- 卖家精灵 `商品与优惠` 功能域调用间隔不得短于 12 秒；如果用户或 MCP 明确提供更低速率，以更低速率为准。
- 本 Skill 禁止调用卖家精灵 `流量词统计`、`流量词明细`、关键词反查或关联流量类接口。自然关键词排名、自然关键词数量、主要关键词搜索趋势只用 SIF MCP；SIF 不可用时 blocked，不得退回卖家精灵流量词接口。
- 卖家精灵接口不得并行批量跑子体。父商品子体较多时必须按本章的“命中即停止”和“方向性排除”规则减少调用量。
- `keepa_info` 调用必须只请求本步骤需要的最小 `returnFields`，不得为省事一次性请求无关字段。
...
执行本章前，如当前运行环境可发现本地全局 Skill、项目规则或知识库 `kuajing-wulaoshi-sellersprite-mcp-database`，先按当前 AI/智能体的平台资源列表读取其数据库说明，确认卖家精灵 MCP 的通用工具边界。不得假定该规则资源位于任何固定目录。通用数据库说明不能覆盖本章为流量异动写死的工具、参数、字段和阻塞条件。
...
- 本项只允许调用 `competitor_lookup`，每个时间窗口最多调用一次；不得因为需要变体数量而调用 `keepa_info`、流量词、关联流量或其他卖家精灵接口。
- 优先使用 `variations`。
...
- 流量增加时，只验证 Deals 促销开始。先扫描分析期子体并集，任一子体命中 Deals 后立即停止分析期扫描；如果分析期全部子体均未命中 Deals，则本项 verified 且未命中，不再扫描对比期。只有分析期已命中 Deals 时，才继续扫描对比期；对比期任一子体命中 Deals 后立即停止对比期扫描。分析期有 Deals 且对比期无 Deals，命中 Deals 促销开始；两期都有 Deals，verified 且未命中。
- 流量减少时，只验证 Deals 促销结束。先扫描对比期子体并集，任一子体命中 Deals 后立即停止对比期扫描；如果对比期全部子体均未命中 Deals，则本项 verified 且未命中，不再扫描分析期。只有对比期已命中 Deals 时，才继续扫描分析期；分析期任一子体命中 Deals 后立即停止分析期扫描。对比期有 Deals 且分析期无 Deals，命中 Deals 促销结束；两期都有 Deals，verified 且未命中。
- 流量方向未确认时，本项 blocked，禁止先跑卖家精灵高频接口。
- 每次 `keepa_info` 调用前必须检查 2.1 运行缓存；同一子体、市场、时间窗口和 `returnFields` 已成功返回时不得重复调用。
- `商品与优惠` 功能域必须串行执行，任意两次 `keepa_info` 调用间隔不得短于 12 秒；遇到限流按 2.1 等待和重试，不得继续请求剩余子体。
...
- Coupon 不在本 Skill 判断。
- 本项最终证据允许输出“已确认至少 1 个子体存在 Deals”和命中子体示例、Deal 日期、Deal 价格区间；不得为了统计完整涉及子体数量而继续调用剩余子体。

阻塞条件：

- `keepa_info` 不可用。
- `dealPrice` 字段缺失。
- 按上述方向性扫描后仍无法完成必要时间窗口验证。
- 触发限流并按 2.1 重试后仍不能继续。
...
- 流量增加时，本项直接按方向性排除写入 verified，`evidence.direction_excluded=true`，不得调用 `keepa_info`。
- 流量减少时，只对当前父商品子体逐个调用。任一子体出现非库存原因的异常状态，即可命中 Listing 状态异常；命中后不得为了统计完整异常子体清单继续调用剩余子体。
- 如果已扫描子体均正常但当前父商品子体尚未全部覆盖，则必须继续按 2.1 的串行、缓存和限流规则完成验证；限流重试后仍不能完成时，本项 blocked。
- 每次 `keepa_info` 调用前必须检查运行缓存；同一子体、市场、`dailyLatest=true` 和 `returnFields` 已成功返回时不得重复调用。
- `商品与优惠` 功能域必须串行执行，任意两次 `keepa_info` 调用间隔不得短于 12 秒。
...
- 本项最终证据允许输出“已确认至少 1 个非库存原因的异常子体”和命中子体示例、当前状态自然语言描述；不得为了统计完整异常子体数量而继续调用剩余子体。

阻塞条件：

- `keepa_info` 不可用。
- `productStatus` 字段缺失。
- 流量减少且未命中异常时，当前父商品子体无法全部查询。
...
- 优先使用 4.3 确认的父 ASIN 调用 SIF。
- 如果 SIF 对父 ASIN 不接受、无数据或无法返回可验证结果，改为按 4.5 构建的 `child_asin_union` 逐个子 ASIN 调用同一工具并合并结果。
- 父 ASIN 不可用时，禁止只取任一子 ASIN 冒充父商品自然搜索结果。
- 同一子 ASIN、市场、时间口径和页码已经成功返回时，必须使用 2.1 运行缓存，不得重复调用。

固定入参：

```json
{
  "asin": "<4.3 确认的父 ASIN；父 ASIN 不可用时填 child_asin_union 中的单个子 ASIN>",
  "country": "<市场>",
  "timePieceType": "month",
...
- 父 ASIN 可用时，父商品自然关键词数量优先使用父 ASIN 调用返回的父商品/Listing 口径结果；如果返回结构拆成子 ASIN 行，则只汇总 4.5 `child_asin_union` 命中的 `list[].natural`。
- 父 ASIN 不可用时，父商品自然关键词数量 = 对 `child_asin_union` 逐个子 ASIN 调用后，所有可验证子体自然关键词数量的汇总；同一子 ASIN 在同一时间口径只计一次。
- 子 ASIN 调用成功但该子体在该时间口径没有可验证自然关键词数据时，内部记录为该子体无 SIF 覆盖，不得用其他子体结果替代；如果全部子体都无可验证数据，标记本项 blocked。
...
- `list[].natural` 缺失。
- 当前期或上期无法取得同口径数据。
- 不能完成父 ASIN 或必要子 ASIN 在该时间口径下的结果分页。
- 父 ASIN 不可用，且 SIF 返回结果无法覆盖父商品子体并集中的任何可验证子 ASIN。
...
- 优先使用 4.3 确认的父 ASIN 调用 SIF。
- 如果 SIF 对父 ASIN 不接受、无数据或无法返回可验证结果，改为按 4.5 构建的 `child_asin_union` 逐个子 ASIN 调用同一工具并合并 `top_keywords[]`。
- 父 ASIN 不可用时，禁止只取任一子 ASIN 的排名变化冒充父商品自然排名变化。
- 同一子 ASIN、市场、时间口径和 `topN` 已经成功返回时，必须使用 2.1 运行缓存，不得重复调用。
...
- 父 ASIN 可用时，直接用父 ASIN 返回的 `top_keywords[]` 和自然排名信号。
- 父 ASIN 不可用时，把 `child_asin_union` 下每个可验证子体返回的 `top_keywords[]` 合并为父商品候选关键词池。
- 合并时按 `keyword` 去重；同一关键词出现在多个子体时，优先保留 `abs(contri_change)` 最大的记录，`contri_change` 缺失时保留 `click_share` 最大的记录，仍无法排序时保留自然排名变化幅度最大的记录。
...
- 当前期或上期无法取得同口径数据。
- 父 ASIN 不可用，且 `child_asin_union` 中没有任何子 ASIN 能返回可验证自然排名数据。
...
- 使用 6.2 已按父商品口径合并后的 `top_keywords[]`，不得重新用任一子 ASIN 单独选词。
- 按 `click_share` 从高到低取前 10 个；`click_share` 缺失时按 `abs(contri_change)` 从高到低取前 10 个。
- 如果少于 10 个，取全部。
...
- 必须输出可核验的业务指标：对比期值、分析期值、变化值、变化率、子体数量或已确认数量下限、异常示例、影响日期、影响天数、链接或页面日期。
